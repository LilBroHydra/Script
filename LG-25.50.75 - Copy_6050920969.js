...
function min() {
    Chat.easyTip('mrcyjanek'); return false;
}
$("#play").on("click", function () {
    ...
    prediction = Math.floor((Math.random() * (98 - 50 + 1)) + 50);
    direction = "under";
    chance = prediction;
    ...
    if (run === true) {
        jQuery.ajax({
		    ...
		    $('#currentMaxProfit').html(largestProfittt.toFixed(8));
                    ...
                    totalProf = totalProf - 0 + ((Number(_0xb2e6x20.profit)*1.01) - 0) - 0;
                                        //prediction = 50, betAmount = 0.00000001, direction = 'under', info
                    if (totalProf > 0) {
                        totalProf = 0
                    }
                    if (win) {
                        nextbet = basebetAmount*Math.random()
                        prediction = 90+(Math.random()*7)
                    }
                    if (lose) {
                        let a = [25,50,75]
                        let methodid = a[(Math.random()*2).toFixed(0)]
                        prediction = methodid
                        nextbet = Number((totalProf*-1)/(Number(under[methodid])-1))*((Math.random()/5)+1)
                    }
                    if (nextbet < basebetAmount) {
                        nextbet = basebetAmount
                    }
                    console.log(`${ Number(prediction).toFixed(8) }, ${ Number(nextbet).toFixed(8) } || ${ Number(totalProf).toFixed(8) }`)
